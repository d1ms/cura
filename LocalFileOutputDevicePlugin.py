...
class LocalFileOutputDevice(OutputDevice):
    def __init__(self):
        ...
        self.setShortDescription(catalog.i18nc("@action:button Preceded by 'Ready to'.", "Save to File 1"))
        self.setDescription(catalog.i18nc("@info:tooltip", "Save to File 3"))
    ...
    def requestWrite(self, nodes, file_name = None, limit_mimetypes = None, file_handler = None):
        ...
        gcode_list = getattr(scene, "gcode_list") #get gcodes array of strings
        ...
        if ".gcode" in file_name:
            resulte = QMessageBox.question(None, catalog.i18nc("@title:window", "Splitting this file" ), catalog.i18nc("@label", "Splitting file <filename>{0}</filename> to "+str(len(gcode_list))+" part?").format(file_name))
            if resulte == QMessageBox.Yes:
               spliter = True
        ...
        nodes.append(-10) #important 
        #QMessageBox.question(None, catalog.i18nc("@title:window", str(len(gcode_list)) ), catalog.i18nc("@label", "The file <filename>{0}</filename> already exists. Are you sure you want to overwrite it?").format(file_name)) 	
        i = 0
        if spliter:
            for gcode in gcode_list: #circle for layer
                try:
                    nodes[1] = i
                    file_name = dialog.selectedFiles()[0].replace(".gcode", str(i) + ".gcode")
